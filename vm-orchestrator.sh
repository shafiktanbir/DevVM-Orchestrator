...
readonly MAX_STARTUP_WAIT="${MAX_STARTUP_WAIT:-120}"  # Increased for first-boot scenarios
readonly SSH_CHECK_INTERVAL="${SSH_CHECK_INTERVAL:-5}" # Slower checks to reduce load
...
start_vm_if_needed() {
    ...
    case "$state" in
        running) 
            ...
            # CRITICAL FIX: Add grace period after "running" state for network/SSH init
            log_info "Waiting 10s for network services to initialize..."
            sleep 10
            ;;
        poweroff|aborted|saved) 
            ...
            log_info "VM state confirmed: running"
            
            # CRITICAL FIX: First-boot grace period (Ubuntu needs time for SSH init)
            log_info "Waiting 15s for SSH service initialization (first-boot safe delay)..."
            sleep 15
            ;;
...
wait_for_ssh() {
    log_info "Waiting for SSH banner on $SSH_HOST:$SSH_PORT (max ${MAX_STARTUP_WAIT}s)..."
    log_info "→ Checking for actual SSH service readiness (not just open port)"
    
    local start_time=$SECONDS
    local banner=""
    
    # CRITICAL FIX: Wait for actual SSH banner (not just open port)
    # This prevents "Connection timed out during banner exchange" errors
    until banner=$(timeout 5 bash -c "echo 'exit' | nc -w 5 $SSH_HOST $SSH_PORT 2>/dev/null | head -1" 2>/dev/null); do
        [[ $((SECONDS - start_time)) -ge MAX_STARTUP_WAIT ]] && {
            ...
            log_error "SSH banner timeout after ${MAX_STARTUP_WAIT}s. VM may be under heavy load or SSH not installed."
        }
        sleep "$SSH_CHECK_INTERVAL"
    done
    ...
    if echo "$banner" | grep -qE "^SSH-2\.0-"; then
        log_info "✓ SSH banner verified: $banner"
...
launch_ssh_terminal() {
    ...
    if ! pgrep -f "xfce4-terminal.*$VM_NAME" >/dev/null 2>&1; then
        log_warn "Terminal may not have launched properly. Check for errors in terminal window."
    else
...
main() {
    ...
    log_info "Config: VM=$VM_NAME | User=$SSH_USER | Host=$SSH_HOST:$SSH_PORT | AutoShutdown=$AUTO_SHUTDOWN"
...
# ==================== SIGNAL HANDLING ====================
cleanup() {
    log_warn "Received interrupt signal - exiting gracefully"
    exit 130
}
